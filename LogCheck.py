...
class LogCheck:
    ...
    def parse_log(self, log):
        ...
        mmio_devirt_print = []
        if l_info.get("mmio_devirt"):
            pad_addr = len(max([x[0] for x in l_info["mmio_devirt"]],key=len))
            pad_size = len(max([x[1] for x in l_info["mmio_devirt"]],key=len))
            pad_page = len(max([x[2] for x in l_info["mmio_devirt"]],key=len))
            for x in l_info["mmio_devirt"]:
